...
public class Profile_Activity extends Activity {
	...
	public _Profile FProfile = new _Profile();
	...
	public void parseData(JSONObject object_response){
		try {
			...
			FProfile.userId 			= ""+jObject_info.get("userId");
			FProfile.avatarPath 		= ""+jObject_info.get("avatarPath");
			FProfile.avatarUrl 			= ""+jObject_info.get("avatarUrl");
			FProfile.hasAvatar 			= ""+jObject_info.get("hasAvatar");
			FProfile.avatarCount 		= ""+jObject_info.get("avatarCount");
			FProfile.avatarName 		= ""+jObject_info.get("avatarName");
			FProfile.fullName 			= ""+jObject_info.get("fullName");
			FProfile.displayName 		= ""+jObject_info.get("displayName");
			FProfile.userName 			= ""+jObject_info.get("userName");
			FProfile.email 				= ""+jObject_info.get("email");
			FProfile.aboutMe 			= ""+jObject_info.get("aboutMe");
			FProfile.postCount 			= ""+jObject_info.get("postCount");
			FProfile.favoriteCount 		= ""+jObject_info.get("favoriteCount");
			FProfile.viewCount 			= ""+jObject_info.get("viewCount");
			FProfile.fCounts.follower 	= ""+jObject_info.getJSONObject("followCounts").get("follower");
			FProfile.fCounts.following 	= ""+jObject_info.getJSONObject("followCounts").get("following");
			   
			FUserId = FProfile.userId;
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		this.headerUIClass.setMenuTitle( FProfile.displayName.length()>0?FProfile.displayName:FProfile.userId );
		//setTitleText(profile.displayName.length()>0?profile.displayName:profile.userId);
		
		String appendHighlight = Integer.parseInt(FProfile.postCount)>1?" highlights":" highlight";
		String appendFollower = Integer.parseInt(FProfile.fCounts.follower)>1?" followers":" follower";
		
		txtvw_highlightCount.setText(FProfile.postCount+appendHighlight);
		txtvw_followingCount.setText(FProfile.fCounts.following+" following");
		txtvw_followerCount.setText(FProfile.fCounts.follower+appendFollower);
		 
		String ngeks = FProfile.avatarUrl;
		String he = ngeks;
		
		/*//ORIGINAL 
			if(Integer.parseInt(FProfile.hasAvatar)>0){
				AQuery aQuery = new AQuery(this); 
				aQuery.id(imgvw_avatar_profile).image(FProfile.avatarUrl, false, true);
			}else{
		...
		if( Integer.parseInt(FProfile.hasAvatar) > 0 ){
			...
			new StreamImageTask( imgvw_avatar_profile,  (ProgressBar) findViewById(R.id.pbLoading_avatar)  ).execute(FProfile.avatarUrl); 
		}else{
	...
	private void addHeaderButtonListener(){
		this.headerUIClass.setOnHeaderButtonClickedListener(new HeaderUIClass.OnHeaderButtonClickedListener() {
			...
			public void onDoneClicked() { 
				...
				intent.putExtra("fullname", FProfile.fullName);
				intent.putExtra("photo", 	FProfile.avatarUrl);
				intent.putExtra("email", 	FProfile.email);
				intent.putExtra("motto", 	FProfile.aboutMe);
				intent.putExtra("avatarCount", Integer.parseInt(FProfile.avatarCount));
